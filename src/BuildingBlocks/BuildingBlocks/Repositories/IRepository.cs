...
public interface IRepository<TContext> where TContext : class
{
    ...
    Task<T?> GetWithIncludesAsync<T>(Expression<Func<T, bool>> filter, Func<IQueryable<T>, IQueryable<T>> includeBuilder, CancellationToken cancellationToken = default) where T : class;
