    ...
    public static BandDto ToBandDto(
        ...
        ILookup<BandId, Album> albumsByBand) => new(
            ...
            albumsByBand[band.Id]
                .Select(a => new AlbumSummaryDto(a.Id.Value, a.Title, a.AlbumRelease.ReleaseYear, a.CoverUrl, a.Type, a.AlbumRelease.Format))
                .ToList(),
            band.BandGenres
                .Select(bg => new GenreDto(genres[bg.GenreId].Id.Value, genres[bg.GenreId].Name, bg.IsPrimary))
                .ToList()
        );
}
